...
// Plan tallennetaan plain textinä
export async function saveSectionFromForm(folderId: string, type: "plan", formData: FormData) {
  const session = await auth();
  const me = requireEmail(session);
  ...
  const title = String(formData.get("title") ?? "").trim() || "Valmennussuunnitelma";
...
// ✅ Tulevat tallennetaan JSON:na (kalenterivalinnat + manualText)
